...
import { getFastSupportForModel, isFastModeRequested, parseFastModeModel } from "./shared/fast-mode.js";
...
async function writeSubagentTempFiles(options: { fastMode: boolean }): Promise<{ dir: string; systemPromptFile: string; fastModeExtensionFile?: string }> {
	...
	if (!options.fastMode) return { dir, systemPromptFile };

	const fastModeExtensionFile = path.join(dir, "fast-mode-bridge.ts");
	await fs.promises.writeFile(
		fastModeExtensionFile,
		[
			"export default function subagentFastModeBridge(pi: any): void {",
	...
	return { dir, systemPromptFile, fastModeExtensionFile };
}
...
function mergeConfig(
	...
	return {
		...
		fastMode: defaults.fastModeRequested && getFastSupportForModel(parseFastModeModel(parsedModel.model)).supported,
	};
}
...
async function runSubagent(
	...
	let tempFiles: { dir: string; systemPromptFile: string; fastModeExtensionFile?: string } | undefined;
	try {
		tempFiles = await writeSubagentTempFiles({ fastMode: config.fastMode });
		const args = ["--mode", "json", "-p", "--no-extensions", "--no-skills", "--append-system-prompt", tempFiles.systemPromptFile];
		// Keep child sessions clean while still carrying the parent's explicit fast-mode preference.
		// Pi's --no-extensions disables discovery; explicit --extension paths remain loaded.
		if (tempFiles.fastModeExtensionFile) args.push("--extension", tempFiles.fastModeExtensionFile);
		...
		if (tempFiles) await fs.promises.rm(tempFiles.dir, { recursive: true, force: true }).catch(() => undefined);
	}
}
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate: OnUpdate, ctx): Promise<ToolResult> {
			...
			const defaults = {
				...
				fastModeRequested: isFastModeRequested(ctx),
			};
			...
			const summaries = results.map((result) => {
				...
				const meta = [`thinking ${result.thinking}`, result.model, result.fastMode ? "fast" : undefined, usage].filter(Boolean).join(" · ");
		...
		renderResult(result, { expanded }, theme) {
			...
			const lines = details.results.flatMap((item) => {
				...
				const meta = [`thinking ${item.thinking}`, item.model, item.fastMode ? "fast" : undefined, usage].filter(Boolean).join(" · ");
